...
namespace engine::render {
    ...
    void Camera::update(float delta_time)
    {
        ...
        glm::vec2 desired_position = target_pos - viewport_size_ / 2.0f;//计算相机的目标的位置，实际上就是相机的期望位置

        auto distance_ = glm::distance(position_, desired_position);
        constexpr float SNAP_THRESHOLD = 1.0f;      //设置一个距离阈值（constexpr：编译时常量，避免每次调用都要计算一次）
